...
class Crawler:
    def __init__(self, input_url_as_str: T_URL, max_number_of_urls: int, crawling_depth: int):
        ...
        self.html_text_as_list: T_HTML_TEXTS = []

    async def get_text_from_url(self) -> None:
        index_for_while = -1
        logging.info(f'start, amount of html text: {len(self.html_text_as_list)}')
                ...
                async with aiohttp.ClientSession() as session:
                    try:
                        async with session.get(self.urls_as_list[index_for_while]) as response:
                            ...
                            self.html_text_as_list.append(response_text)
                            logging.info(f'amount of html text: {len(self.html_text_as_list)}')
                    except IndexError:
                        index_for_while -= 1
                        continue
                    if index_for_while == self.crawling_depth - 1:
                        break
            logging.info(f'end, amount of html text: {len(self.html_text_as_list)}')

    async def get_urls_as_list(self) -> None:
        index_for_while = -1
        while_bool = True
        ...
        while while_bool:
            ...
            index_for_while += 1
            try:
                soup = bs4.BeautifulSoup(markup=self.html_text_as_list[index_for_while], features='html.parser')
            except IndexError:
                index_for_while -= 1
                continue
            try:
                ...
                continue
            if self.crawling_depth == len(self.html_text_as_list):
                break
        logging.info(f'end, amount of urls: {len(self.urls_as_list)}')

    async def main(self):
        tasks = [
            Crawler.get_text_from_url(self),
            Crawler.get_urls_as_list(self)
        ]
        ...
        _added_urls_in_json_file(file_name=file_name, urls=self.urls_as_list)
